...
class TicketModelTest(TestCase):
    ...
    def test_ticket_k_number_not_unique(self):
        """Test that K-Number can be duplicated (multiple tickets per user allowed)"""
        ticket1 = Ticket.objects.create(**self.ticket_data)
        ticket2 = Ticket.objects.create(**self.ticket_data)
        # Both tickets should be created successfully
        self.assertEqual(ticket1.k_number, ticket2.k_number)
